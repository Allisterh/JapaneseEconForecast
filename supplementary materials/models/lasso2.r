# lasso with regularisation parameter selected by cv, rolling window. Lag = 2
...
X <- lag.xts(dat, 1:2+h-1)

predErr <-
  foreach(t = 1:winSize, .combine = "cbind", .inorder = F) %dopar% {
    fit <- glmnet::glmnet(X[(2+h):T1+t-1,],y[(2+h):T1+t-1], # (p+h):T1 instead of 1:T1 bc first p obs's are missing
...
LASSO4lambda[horizon,targetVar] <- optLam
...
LASSO4sparsityRatio[horizon,targetVar] <- mean(coefTracker) # the ratio of non-zero coef
LASSO4nonzero[horizon,targetVar] <- sum(coefTracker)/winSize # avg nr of non-zero coef per window

# Notice that the final object `LASSOcoefs` is a list of lists (main list of variables and sub-list of horizons)
if (horizon == 1) {LASSO4coefs[[var]] <- list()} # initialise by setting sub-list so that each main list contains sub-lists
LASSO4coefs[[var]][[horizon]] <- coefTracker
if (horizon == 4) {names(LASSO4coefs[[var]]) <- paste("h", hChoises, sep="")}

MSFEs[[horizon]]["LASSO4", targetVar] <- msfeLasso
