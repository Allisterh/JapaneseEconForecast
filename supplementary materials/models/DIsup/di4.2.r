...
if (var==1){ # Factors are identical for the same horizon 
  ...
  for (t in 1:winSize){
    ...
    DI4.2factorList[[horizon]][[t]] <- xts(Fhat, order.by = index(x))
  }
} # endif
...
if (horizon==1){ # create placeholders inside each lists
  DI4.2lags[[var]] <- matrix(NA, nrow=length(hChoises), ncol=winSize, 
                           dimnames = list(c(paste("h=",hChoises,sep=""))))
}
DI4.2lags[[var]][horizon,] <- unlist(sapply(eval, function(foo) foo[2]))
MSFEs[[horizon]]["DI4.2", targetVar] <- mean(predErr)
