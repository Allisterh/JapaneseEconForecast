...
if (var==1){ # Factors are identical for the same horizon 
  ...
  DI6.2factorList[[horizon]] <- list()
  for (t in 1:winSize){
    x <- datLag[(T1-1):T2+t,] # to estimate factor structure
    N <- ncol(x)
    T <- nrow(x)
    eig <- eigen(x%*%t(x) / (T*N))
    vec <- eig$vectors
    ...
    DI6.2factorList[[horizon]][[t]] <- xts(Fhat, order.by = index(x))
  }
} # endif
...
if (horizon==1){ # create placeholders inside each lists
  DI6.2lags[[var]] <- matrix(NA, nrow=length(hChoises), ncol=winSize, 
                             dimnames = list(c(paste("h=",hChoises,sep=""))))
}
DI6.2lags[[var]][horizon,] <- unlist(sapply(eval, function(foo) foo[2]))
MSFEs[[horizon]]["DI6.2", targetVar] <- mean(predErr)
