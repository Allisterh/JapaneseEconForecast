...
if (var==1){ # Factors are identical for the same horizon 
  ...
  DIfullfactorList[[horizon]] <- list()
  for (t in 1:winSize){
    x <- datLag[(h+1):T2+t,] # to estimate factor structure
    N <- ncol(x)
    T <- nrow(x)
    eig <- eigen(x%*%t(x) / (T*N))
    vec <- eig$vectors
    ...
    DIfullfactor[horizon,t] <- optFac # keep track of nr-of-factors 
    DIfullfactorR2[horizon,t] <- varianceRetained[optFac]
    ...
    DIfullfactorList[[horizon]][[t]] <- xts(Fhat, order.by = index(x))
    ...
    DIfullfactorDyn[horizon,t] <- which.min(ICdyn)
  }
} # endif
...
eval <-
  foreach(t=1:winSize) %dopar% {
    Fhat <- DIfullfactorList[[horizon]][[t]]
    bic<-1e10
    for (p in 1:12){
...
if (horizon==1){ # create placeholders inside each lists
  DIfulllags[[var]] <- matrix(NA, nrow=length(hChoises), ncol=winSize, 
                          dimnames = list(c(paste("h=",hChoises,sep=""))))
}
DIfulllags[[var]][horizon,] <- unlist(sapply(eval, function(foo) foo[2]))
MSFEs[[horizon]]["DIfull", targetVar] <- mean(predErr)
