...
## In this prototype, I just focus on forecasting inflation (dat[,51])
...
#### AR ####
# univariate AR with flexible lag selection
source("ar.r") 
# univariate AR with fixed lag length selected by cross validation
source("ar2.r")
...
# univariate (baseline) ---------------------------------------------------
# T2 <- 2*(floor(nrow(dat)/3)) # evaluate the best lambda 
# # start<-which(index(dat)=="1995-06-01") # last observation for first forecast
# T <- nrow(dat) - 1 # last observation for the last forecast (2001-02-01)
# predErrAR<- 0
# foo<- numeric()
# idxTracker<-0
# for (t in T2:T){
...
for (p in 1:12){   # loop through lags 
  ...
  for (t in T2:(end-1)){ # forecast evaluation
    fitLasso <- glmnet(x[1:t], y[1:t], lambda=12,family = "gaussian", alpha=1)
    predLasso <- predict.glmnet(fitLasso, coredata(x[t+1,]))
