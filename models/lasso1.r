...
X <- lag.xts(dat, h) # lag=4

predErr <-
  foreach(t = 1:winSize, .combine = "cbind", .inorder = F) %dopar% {
    fit <- glmnet(X[(1+h):T1+t-1,],y[(1+h):T1+t-1], # (p+h):T1 instead of 1:T1 bc first p obs's are missing
...
coefTracker <- matrix(unlist(sapply(eval, function(foo) foo[2])),
                      nrow=winSize, ncol=ncol(X), byrow=T) # 60x508 matrix
...
MSFE1[[horizon]]["LASSO", targetVar] <- msfeLasso
