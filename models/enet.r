...
lambdaChoises <- 10^(seq(1.5,-3,len=100)) # lambda choices (or use the optLam from LASSO to reduce time)
...
cvScore <-  # nr-of-lambda x nr-of-alpha
  foreach(a = 1:length(alphaChoises), .combine = "cbind", .inorder = F) %dopar% {
    for (t in 1:winSize){
      fitEnet <- glmnet::glmnet(X[(12+h):T1+t-1,],y[(12+h):T1+t-1], # (p+h):T1 instead of 1:T1 bc first p obs's are missing
